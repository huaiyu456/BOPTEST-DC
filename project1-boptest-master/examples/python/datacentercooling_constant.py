...
with the prototype test case called "SingleChillerPlantWSE".  It uses the testing
interface implemented in interface.py and the concrete controller implemented
in controllers/datacentercooling_constant.py.
...
def run(plot=False):
    ...
    # ---------------------------------------
    control_module = 'examples.python.controllers.datecenterconstant'
    start_time = 0
    warmup_period = 0
    length = 48*3600
    step = 300
    ...
    # --------------------
    kpi, df_res, custom_kpi_result, forecasts = control_test('SingleChillerPlantWSE',
                                                             control_module,
    ...
    zone_temperature = df_res['TAirSup_y'].values - 273.15  # convert K --> C
    energy_all = df_res['PAll_y'].values
    ...
    if plot:
        try:
            ...
            plt.xlabel('Time [hr]')
            plt.figure(2)
            plt.title('energy_all')
            plt.plot(time, energy_all)

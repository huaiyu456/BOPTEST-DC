...
model TestCase "Multi zone residential hydronic example model"
  ...
  parameter Modelica.Units.SI.Temperature dTBoi_nominal=TSup_nominal -
      TRet_nominal "Nominal temperature difference for boiler loop"
  ...
  parameter Modelica.Units.SI.MassFlowRate mBoi_flow_nominal=scaFacRad*
    ...
    annotation (Dialog(group="Boiler"));
  parameter Modelica.Units.SI.Power P_pum_nominal=45 "Pump electrical power"
    annotation (Dialog(group="Boiler"));
  parameter Modelica.Units.SI.Power Pmin_Ch=800 "Boiler minimum power"
    annotation (Dialog(group="Chauffage"));
  parameter Modelica.Units.SI.Power Pmax_Ch=Q_flow_nominal
    "Boiler maximum power" annotation (Dialog(group="Chauffage"));
  parameter Modelica.Units.SI.Time t_anticourtcycle=600
    "Anti short cycle duration for the boiler"
    annotation (Dialog(group="Cractristiques de la chaudire"));

  // Radiators
  parameter Real delta_ST_rad = 0 "Coefficient for spatial variation of the control system" annotation(Dialog(group = "Radiators"));
  parameter Modelica.Units.SI.Power Q_rad_liv=1800 "Living room radiator power"
    annotation (Dialog(group="Radiators"));
  parameter Modelica.Units.SI.Power Q_rad_ro1=1000 "Room 1 radiator power"
    annotation (Dialog(group="Radiators"));
  parameter Modelica.Units.SI.Power Q_rad_ro2=900 "Room 2 radiator power2"
    annotation (Dialog(group="Radiators"));
  parameter Modelica.Units.SI.Power Q_rad_ro3=1100 "Room 3 radiator power"
    annotation (Dialog(group="Radiators"));
  parameter Modelica.Units.SI.Power Q_rad_bth=800 "Bathroom radiator power"
    annotation (Dialog(group="Radiators"));
  parameter Modelica.Units.SI.Power Q_rad_hal=1000 "Hall radiator power"
    annotation (Dialog(group="Radiators"));
...
//  Modelica.SIunits.Power Consommation_Gaz_Chaudiere "Boiler gas consumption";
  Modelica.Units.SI.Power Consommation_Elec_Pompe
    "Boiler pump electricity consumption";

protected
  parameter Modelica.Units.SI.Length HSP=2.57 "Ceiling height";
  parameter Modelica.Units.SI.Length H_Combles=1.84 "Attic height";
  parameter Modelica.Units.SI.ThermalConductivity k_PT=9.7/42.8
    "Thermal bridges coefficient";
  parameter Modelica.Units.SI.Density d_air=1.184 "Air density";
  parameter Modelica.Units.SI.MassFlowRate Q_batiment=113.4/3600*d_air
    "Global mechanical ventilation airflow, this is approximately 1 m3/h/m2 ";
  ...
  parameter Modelica.Units.SI.Area S_ref=100 "Buildign rerefence surface";
                                  // used to scale up the actual building and take into account a bigger building surface
  ...
  parameter Modelica.Units.SI.Length L_ext_Salon=(Spe_Salon_Nord +
      Spi_Salon_Garage + Spe_Salon_Est + Spe_Salon_Sud)/HSP;
  ...
  parameter Modelica.Units.SI.MassFlowRate Q_Salon=Q_batiment*S_Salon/(S_Salon
       + S_Chambre1 + S_Chambre2 + S_Chambre3);
  ...
  parameter Modelica.Units.SI.MassFlowRate Q_Chambre1=Q_batiment*S_Chambre1/(
      S_Salon + S_Chambre1 + S_Chambre2 + S_Chambre3);

  // Room 2
  parameter Modelica.Units.SI.Area S_Chambre2=9.85*S/S_ref;
  parameter Modelica.Units.SI.Length L_ext_Chambre2=(Spe_Chambre2_Sud +
      Spe_Chambre2_Ouest)/HSP;
  ...
  parameter Modelica.Units.SI.MassFlowRate Q_Chambre2=Q_batiment*S_Chambre2/(
      S_Salon + S_Chambre1 + S_Chambre2 + S_Chambre3);

  // Room 3
  parameter Modelica.Units.SI.Area S_Chambre3=14.28*S/S_ref;
  parameter Modelica.Units.SI.Length L_ext_Chambre3=(Spe_Chambre3_Ouest +
      Spe_Chambre3_Nord)/HSP;
  ...
  parameter Modelica.Units.SI.MassFlowRate Q_Chambre3=Q_batiment*S_Chambre3/(
      S_Salon + S_Chambre1 + S_Chambre2 + S_Chambre3);
  ...
  parameter Modelica.Units.SI.Area Spe_Combles_exterieur_Nord=sqrt(H_Combles^2
       + (8*sqrt(S/S_ref))^2)*(12.7*sqrt(S/S_ref));
  ...
  parameter Modelica.Units.SI.Area Spe_Combles_exterieur_Sud=
      Spe_Combles_exterieur_Nord;
  ...
  parameter Modelica.Units.SI.MassFlowRate Q_Combles=0.5*S_Combles*H_Combles/2/
      3600*d_air;

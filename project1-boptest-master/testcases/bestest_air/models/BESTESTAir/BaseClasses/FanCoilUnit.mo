...
model FanCoilUnit "Four-pipe fan coil unit model"
  ...
  parameter Modelica.Units.SI.MassFlowRate mAir_flow_nominal=0.55
    "Nominal air flowrate" annotation (Dialog(group="Air"));
  parameter Modelica.Units.SI.DimensionlessRatio minSpe=0.2 "Minimum fan speed"
    annotation (Dialog(group="Air"));
  parameter Modelica.Units.SI.Power QCooCap=3666 "Cooling coil capacity"
    annotation (Dialog(group="Coils"));
  parameter Modelica.Units.SI.Power QHeaCap=7000 "Heating coil capacity"
    annotation (Dialog(group="Coils"));
  parameter Modelica.Units.SI.DimensionlessRatio COP=3
    "Assumed COP of chiller supplying chilled water to FCU in [W_thermal/W_electric]"
    annotation (Dialog(group="Plant"));
  parameter Modelica.Units.SI.DimensionlessRatio eff=0.9
    "Assumed efficiency of gas boiler supplying hot water to FCU in [W_gas/W_thermal]"
    annotation (Dialog(group="Plant"));
  final parameter Modelica.Units.SI.Pressure dpAir_nominal=185
    "Nominal supply air pressure";
  final parameter Modelica.Units.SI.MassFlowRate mCoo_flow_nominal=QCooCap/(
      4200*5) "Nominal chilled water flowrate";
  final parameter Modelica.Units.SI.MassFlowRate mHea_flow_nominal=QHeaCap/(
      4200*20) "Nominal heating water flowrate";
  final parameter Modelica.Units.SI.Pressure dpCoo_nominal=((mCoo_flow_nominal/
      1000)*3600/(0.865*1))^2*1e5 "Nominal chilled water pressure drop";
  final parameter Modelica.Units.SI.Pressure dpHea_nominal=((mHea_flow_nominal/
      1000)*3600/(0.865*1))^2*1e5 "Nominal heating water pressure drop";
